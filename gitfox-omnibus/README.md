...
运行 `gitfox init` 会在数据目录生成两个配置文件，并**自动生成密钥**：
...
| 配置方式 | .env + TOML 文件 | gitlab.rb (Ruby DSL) |
| 配置管理 | `gitfox init` 生成模板 | `gitlab-ctl reconfigure` |
| 大小 | ~120MB | ~1GB |
| 启动速度 | <3s (首次解压) | ~30s |
