...
│    4. 收集 migrations/*.sql                                      │
│    5. (可选) 构建内置依赖 (PostgreSQL, Redis, Nginx)              │
│    6. 生成 stub 程序源码                                          │
│    7. 编译 stub → 最终超级二进制                                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
- **可选内置 PostgreSQL**：无需单独安装数据库
- **可选内置 Redis**：无需单独安装缓存服务
- **可选内置 Nginx**：内置前端反向代理，支持 HTTPS
...
如果你想要一个完全自包含的部署（类似 GitLab Omnibus），可以构建包含内置 PostgreSQL、Redis、Nginx 的版本：

```bash
# 1. 构建包含内置依赖的版本
./gitfox-omnibus build --bundled-deps --output /usr/local/bin/gitfox
...
**注意**: 使用内置依赖时，`database.url` 和 `redis.url` 配置将被忽略。
