...
| 用户信息端点 | `GET /api/v1/auth/me` | 获取当前用户信息 |
...
目前 GitFox 不提供主动撤销端点。访问令牌会在过期后自动失效。你可以在数据库中设置 `revoked_at` 字段手动撤销。
...
当前版本 GitFox 实现了标准的 OAuth2，尚未完全实现 OIDC。计划在未来版本中添加 OIDC 支持。
