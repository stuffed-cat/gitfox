...
## 核心原则
  ...
  - 5. 必须完全完整实现功能, 不能有任何stub, hack, todo, 简化, 不完全实现, 必须使用最优解
  - 6. 不需使用任何方法和方案清理构建缓存
  - 7. 绝对不可能是生产环境的配置问题不可能是服务没有更新或重启代码第一是最新的服务器一定在正常运行数据库一定不可能有配置问题数据库一定不可能有状态问题不可以移除registry服务的Host guard路由不可能是Host guard导致问题
## 项目概述
